...
process.on('uncaughtException', (err) => {
  // this events not gonna happend async so we don't need the server at all
...
app.use(helmet());
// its good to put it early in the midlware stack so this headers are really sure to be set
...
app.use(
  hpp({
    // used to clear up the query string and deal with duplicated fields and only accept the last value of the field
    whitelist: [
      'duration',
      'ratingsAverage',
      'ratingsQuantity',
      ...
      'difficulty',
      'price',
    ], // array of properities which we actually  allow duplicates in the query string
  }),
);
...
process.on('unhandledRejection', (err) => {
  ...
  server.close(() => {
    // server.close is to give the server time to finish all the requests that is still pending or being handled at the time and only after this we kill the process
